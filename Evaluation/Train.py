...
### LOAD DATA
dir = r"C:\Users\night\RRL-Stock-Trading\Stock Tickers\Final Data\15min" # local path
name = 'Deep Evolution Strategy' 
name2 = 'AAPL' # ticker name
# path = f'{dir}\{name2}_Daily*.csv' # 15min, 30min, 1hr, daily
path = f'{dir}\{name2}_15min*.csv' # 15min, 30min, 1hr, daily
...
start = '2010-01' ## YYYY-MM
end = '2020-01' ## YYYY-MM

df_full = df_full.loc[start:end].copy()
print(f"No. of data points in {name2} test dataset: {len(df_full)}")

# df_full.drop(['symbol'], axis=1, inplace=True) # use this for daily timeframe ONLY
...
### SCALING
# df_full = df_full.iloc[:, 1:6]
# trans = RobustScaler()
# data = trans.fit_transform(df_full)
# df_full = pd.DataFrame(data)
# df_full.columns = ["Close", "High", "Low", "Open", "Volume"]
...
### FEATURE ENGINEER
# close_noscale = df_full.Close.values.tolist()
# print(close_noscale)
df= df_full.copy()
...
# print("window size:10",get_state(close, 5, 1))
...
#         plt.title('total gains %f, total investment %f%%'%(initial_money - starting_money, invest))
        ...
        plt.legend()
        plt.savefig(f"{model_name}_{ticker}_{max_buy}_Train")
        plt.show()
...
save_weights = True # train version
load = False # train version
load_file = ""
