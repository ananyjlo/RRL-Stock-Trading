...
def get_date(temp_time):
    temp_time = pd.to_datetime(temp_time)
...
def reward_function(X, miu, delta, M, para, threshold, time_index, origin_asset, close, volume):
    Ft = update_position(X, para, threshold, close, volume)
...
def calc_dSdw(X, miu, delta, M, theta, threshold, time_index, origin_asset, close, volume):
    Ret, sharp, asset_list, Ft, pnl, day_index = reward_function(X, miu, delta, M, theta, threshold, time_index, origin_asset, close, volume)
    print("asset")
...
def update_position(origin_data, para, threshold, close, volume, window=288, ma1=72, ma2=288, ma3=288*3):
    m = 5
    ...
    obv_list = calc_obv(close, volume)
...
def main():
    # ticker = pd.read_csv(r'C:\Users\night\RRL-Stock-Trading\Stock Tickers\OHLC Data\C_15min_OHLC.csv', index_col=0)
    ticker = pd.read_csv("C_15min_OHLC.csv")
    p_close = ticker['close']
    p_open = ticker['open']
    p_volumn = ticker['volume']
    # time_str = ticker['time']
    # time_list = [pd.to_datetime(temp) for temp in time_str.values] # original 
    # time_list = [datetime.strptime(x, '%d/%m/%Y %H:%M:%S') for x in ticker['time'] ]
    
    time_list = ticker['time']
    ...
    for loop_i in range(max_loop):
        ...
        # write sharpe
        f_sharpe = open("sharpe_log.txt", 'a')
